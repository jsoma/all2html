...
- Extracted asset `path` values are **relative to `settings.imageOutputPath`**, exactly like Illustrator and the SVG importer. The output directory is applied twice and the two must agree: `export.ts` reads it once and passes it as the emitters' `assetBase` (which prefixes the emitted `src`) and as `createOutputBundle({ assetRoot })` (which prefixes the ZIP layout). Never bake a directory into the asset path.
