...
> **31 DEAD cells recorded; 20 still dead.** Every one is a place where a user sets something, the export succeeds, and nothing happens. Two are closed rather than declared: **D10** (`output` on Illustrator) — `group-artboards.ts` is ES3-safe now, `src/extendscript/index.ts` calls it, and `exporter.jsx` writes one file per group — and **D31** (`useLazyLoader` on video), which `src/emitters/shared/lazy-video.ts` fixed by shipping the loader on all four formats. Nine more are closed by **deleting their settings outright**: `writeImageFiles` (D3–D5), the document-level `inlineSvg` setting (D16–D18), and `svgIdPrefix` (D19–D21) were accepted on every surface and read on none, so the settings are gone from `SETTING_DEFINITIONS`, the schema, the exporter key maps, and the panel (the per-layer `Layer.inlineSvg` flag is unaffected). Matrix footnote 7 (Figma `imageOutputPath`) closed in the same pass and was never a numbered cell. This is still the single largest source of user-visible wrongness in the product, and it is the reason SPEC §12.5 (capability declarations) leads the contract work.
>
> **STATUS: seeded into code.** The 20 live DEAD cells are declared in `src/core/capabilities.ts` and warn at export time with the `setting:unsupported` code. `test/unit/capabilities.test.ts` transcribes the table below (by D-id), asserts the list is exactly 20 entries long, and fails if any of them stops warning; it also re-derives the "zero readers" claims for `createPromoImage` and the *absence* of the three deleted settings from the source tree, so the matrix cannot silently drift from the code. The count appears in three places — this header, that assertion, and the Known Broken bullet in the root `CLAUDE.md` — and all three must move together. Changing a cell here means changing the declaration.
>
...
| 2 | ~~`writeImageFiles`~~ | deleted [D3] | — | deleted [D4] | deleted [D5] |
...
| 27 | ~~`inlineSvg` (setting)~~ | deleted [D16] | — | deleted [D17] | deleted [D18] |
| 28 | ~~`svgIdPrefix`~~ | deleted [D19] | — | deleted [D20] | deleted [D21] |
...
| D3–D5 | ~~everywhere~~ | **Resolved by deletion.** `writeImageFiles` had zero readers on any surface (`illustrator/exporter.jsx#exportImages` runs unconditionally; assets are always written to the Figma ZIP; `createOutputBundle` writes assets unconditionally), so the setting was removed from `SETTING_DEFINITIONS`, the schema, the exporter key map, and the panel. |
| D6–D9 | `figma/src/config.ts#SettingsSchema` | Figma hardcodes transparency, has no quantizer, never emits JPEG, and sets no `constraint` on any `exportAsync`, so the documented default `{type:"SCALE",value:1}` applies (`ExportSettingsImage` in `@figma/plugin-typings`; the recorded params are `runtime-extract.ts#FIGMA_PNG_EXPORT_PARAMS`). D6, D7 and D9 diverge **at the default**: the defaults promise opaque, 128-color, 2x, and Figma produces alpha, full-color, 1x, so all three warn on every export. D8 does not: Figma never emits JPEG at all, so `jpgQuality` is unobservable rather than wrong and warns only when the user moves it. |
...
| D13 | `ui.html#renderTextAs`; also set by the `image-only-graphic` preset (`figma/src/ui.ts#getPresetControls`) | The setting has zero readers: `figma/src/runtime-extract.ts#buildDefaultLayer` derives the disposition from the frame's image-only token alone (`runtime-extract.ts#rendersTextIntoBackground`). Image-only frames record text with `renderAs:"image"` / `renderAsReason:"imageOnly"` and keep it visible for the background raster; every other frame hides text before raster (`runtime-extract.ts#hideTextNodes`) and emits `renderAs:"html"`. |
...
| D16–D18 | ~~everywhere~~ | **Resolved by deletion.** No reader of `settings.inlineSvg` ever existed — emitters read only the **per-layer** flag (`html-tree.ts#inlineSvg`), which remains. The document-level setting, its panel control, and its exporter key-map entry are gone. |
| D19–D21 | ~~everywhere~~ | **Resolved by deletion.** Zero readers. The only prefixing implementation was `src/core/svg-postprocess.ts`, which had **zero importers repo-wide** and was deleted under D16; nothing replaced it, so the setting itself was removed rather than left warning forever. Re-implementing means writing prefixing into the emitter that mints the ids and re-adding the setting with real per-surface declarations. |
...
| D28 | `illustrator/exporter.jsx#local_preview_template` | Only consumer is `standalone.ts#localPreviewTemplate`; `src/extendscript/index.ts#emitHTML` is the only emitter Illustrator imports and it never touches the registry, so standalone is unreachable from Illustrator. |
...
|---|---|---|---|---|
| Format `html` | yes (**only** format; hardcoded `src/extendscript/index.ts#emitHTML`) | no¹⁰ | yes | yes |
...
4. **The `emit` config block** — `fitMode`, `positionMode: "percentage"`, `allowUnsafeHtml: false`, React `typescript`. **Figma reaches it now**: `figma/src/config.ts#EmitterConfigSchema` parses `emit` with the canonical schema — the same one the CLI reads, not a plugin-local shape — and `export.ts` forwards it as the third argument to `emitAll`. **Illustrator still does not**: `src/extendscript/index.ts#emitHTML` is called as `emitHTML(ready, { artboards, slug })` with no emitter options, so the block is unreachable from the production surface.
5. **Direct-call vs registry reachability.** Every surface calls the same `emitHTML`, but Illustrator calls it directly (`src/extendscript/index.ts#emitHTML`) while CLI, browser, and Figma dispatch through the emitter registry. Any capability added only to the registry path is structurally unreachable from Illustrator.
...
- Several tracked `ir.json` files carry raw snake_case settings (`countries`, `fixed`, `template`, `text-cleanup`, `sample-ai-file`) that predate the camelCase rule and are silently ignored by `resolveSettings`. Not used as evidence for any cell.
