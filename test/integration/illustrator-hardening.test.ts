...
> = {
  ...
  "large-story": (raw, html) => {
    ...
    expect(html).toContain("large-story-story-1-dynamic-640-locator.svg");
    expect(html).toContain("large-story-story-6-dynamic-1280-highlight.png");
...
describe("Illustrator hardening fixtures", () => {
  ...
  for (const fixture of illustratorFixtures) {
    it(`${fixture.name} exports, validates and satisfies its assertions`, () => {
      ...
      // Every asset the HTML references must exist in the saved output. The
      // exporter once derived the written SVG filename and the IR asset path
      ...
      if (fixture.releaseBlocking) {
        ...
        for (const match of html.matchAll(/(?:src|data-src)="([^"]+)"/g)) {
          const ref = match[1];
          if (ref.startsWith("data:") || ref.includes("://")) continue;
          const filename = ref.split("/").filter(Boolean).pop() ?? ref;
          expect(
            existsSync(resolve(outputDir, filename)),
            `${fixture.name} HTML references ${ref}, but ${filename} does not exist in ${outputDir}`,
          ).toBe(true);
        }
      }
