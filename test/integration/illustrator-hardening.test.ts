...
> = {
  ...
  "large-story": (raw, html) => {
    ...
    for (const artboard of artboards) {
      expect(
        artboard.breakpoint.maxWidth === Infinity ||
...
describe("Illustrator hardening fixtures", () => {
  for (const fixture of illustratorFixtures) {
    describe(fixture.name, () => {
      it("has complete release-blocking artifacts when required", () => {
        if (!fixture.releaseBlocking) return;

        expect(existsSync(resolve(rootDir, fixture.sourceAiPath))).toBe(true);

        const outputDir = getIllustratorFixtureOutputDir(rootDir, fixture);
        expect(existsSync(outputDir)).toBe(true);
        expect(existsSync(resolve(outputDir, "ir.json"))).toBe(true);
      });

      it("golden IR validates, processes, and emits", () => {
        if (!fixture.requiredArtifacts.goldenIr) return;
      ...
      it("runs fixture-specific assertions when defined", () => {
        const assertion = fixtureAssertions[fixture.name];
        if (!assertion) return;

        const raw = loadGoldenFixture(fixture);
        const { document: processed } = processDocument(raw);
        const { html } = emitHTML(processed);
        assertion(raw, html, loadFixtureSummary(fixture));
      });
    });
  }
});
