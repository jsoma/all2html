...
- `assets.ts` — Asset indexing by canonical `artboardId`/`layerId`, `resolveAssetPath()` (static), `tokenizedAssetPath()` (with `%%ASSET_PATH%%`), `replaceAssetPathToken()`. `resolveAssetPath()` reads `settings.imageSourcePath` (the user's `<img src>` prefix, used verbatim) and the `assetBase` emitter option (the surface's layout: where it writes assets relative to the emitted file). It must never read `imageOutputPath` — that is where the *files* go, which equals the `src` prefix only on surfaces that emit into the root of that layout.
